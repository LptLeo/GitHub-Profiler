...
const Repositories = ({ userData }) => {
  ...
  const filteredRepos = userData ? userData.repositories.nodes.filter(repo =>
      ...
      {userData && (
        <>
          <div className="filter">
            <input type="text" className="filter-input" placeholder="Filtrar" onChange={e => setFilterInput(e.target.value)} />
          </div>
          <div className="row">
            {filteredRepos.length > 0 ? (
              ...
              <div className="insert-message">
                <span className="text-warning fs-1">Repositório não encontrado</span>
              </div>
            )}
          </div>
        </>
